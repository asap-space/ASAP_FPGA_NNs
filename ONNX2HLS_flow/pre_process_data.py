...
def check_file_save(file, numpy_data):
    ...
    with open(file, "rb") as file:
        file_data = np.frombuffer(file.read(), dtype=numpy_data.dtype).reshape(
            numpy_data.shape
        )

    assert np.array_equal(
        numpy_data, file_data
    ), "The data in the file does not match the original numpy data"
...
def write_binary_labels(date):
    ...
    output_file = f"data_binary/{date}/labels_{date}.bin"
    with open(output_file, "wb") as file:
        file.write(label[:].tobytes())
    check_file_save(output_file, label[:])
    print(f"The saved labels are: {label[:]}")
    return cdf[f"epoch_mms1_fpi_fast_dis_dist_{date}"][:]


if __name__ == "__main__":
    os.makedirs(f"data_binary/{DATE}", exist_ok=True)
